...
func (db *DB) NewWriteBatch(opts WriteBatchOptions) *WriteBatch {
	// 使用B+树索引，且事务序列号文件不存咋，且也不是第一次加载数据目录，此时我们的WriteBatch只能禁用b
...
func (wb *WriteBatch) Commit() error {
	...
	// 开始写数据到数据文件当中
	// 由于是一整批写完之后再实际写入数据，所以每次得到的 logRecordPos 可以先暂存起来
	positions := make(map[string]*data.LogRecordPos)
	for _, record := range wb.pendingWrites {
		logReordPos, err := wb.db.appendLogRecord(&data.LogRecord{
		...
		positions[string(record.Key)] = logReordPos
	}
...
// parseLogRecordKey 的 key，获取实际的 key 和事务序列号
